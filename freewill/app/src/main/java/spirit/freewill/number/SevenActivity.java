...
//        addNine();
    ...
    public void addChakra1(){
        DotView chakra1 = new  DotView(SevenActivity.this,6000,12);
...
//        root.addView(nineView);//
        for(int i = 1; i < 10 ; i++){
            OneView oneView = new  OneView(SevenActivity.this,durations[i-1],i);
            root.addView(oneView);//
        }
    }

    public void addBG(){
        BGView bgView = new  BGView(SevenActivity.this);
        menu.addView(bgView);//
        TJView tjView = new  TJView(SevenActivity.this);
        menu.addView(tjView);//
        YJView yjView = new  YJView(SevenActivity.this);
        menu.addView(yjView);//
    }
}

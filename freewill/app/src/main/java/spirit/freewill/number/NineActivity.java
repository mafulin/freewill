...
//    private static long[] durations =  {100,200,300,400,500,600,700,800,900};
    @Override
...
//        addBG();
    ...
    public void addChakra1(){
        DotView chakra1 = new  DotView(NineActivity.this,6000,12);
...
//        root.addView(nineView);//
        for(int i = 1; i < 10 ; i++){
            OneView oneView = new  OneView(NineActivity.this,durations[i-1],i);
            root.addView(oneView);//
        }
    }

    public void addBG(){
        BGView bgView = new  BGView(NineActivity.this);
        menu.addView(bgView);//
        TJView tjView = new  TJView(NineActivity.this);
        menu.addView(tjView);//
    }
}
